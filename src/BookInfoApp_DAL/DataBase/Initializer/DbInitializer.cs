...
namespace BookInfo_DAL.DataBase.Initializer
{
    public class DbInitializer : IDbInitializer
    {
        ...
        private async Task<List<Book>> BookInit(List<Book> bookWorks, List<Author> authors, 
            List<AgeCategory> ageCategories, List<Genre> genres)
        {
            ...
            await SaveeOperation(books, bookRepository);
            ...
            await SaveeOperation(bookAuthors, bookAuthorRepository);
            ...
            await SaveeOperation(bookGenres, bookGenreRepository);
            ...
            await SaveeOperation(inputWorks, inputWorkRepository);
            logger.LogTrace("Сохранение книг");
            return books;
        }

        private async Task<List<Book>> BookWorkInit(List<Author> authors, List<AgeCategory> ageCategories, List<Genre> genres)
        {
            var books = new List<Book>();
        ...
        private async Task<List<Author>> AuthorInit()
        {
            ...
            await SaveeOperation(authors, authorRepository);
            logger.LogTrace("Сохранение писателей");
            return authors;
        }

        private async Task<List<AgeCategory>> AgeCategoryInit()
        {
            ...
            await SaveeOperation(ageCategories, ageCategoryRepository);
            logger.LogTrace("Сохранение ограничение по возрасту");
            return ageCategories;
        }
        private async Task<List<Genre>> GenreInit()
        {
            ...
            await SaveeOperation(genres, genreRepository);
            logger.LogTrace("Сохранение жанров");
            return genres;
        }

        private async Task BookPublisherInit(List<Book> books, List<CoverType> coverTypes, List<Publisher> publishers)
        {
            ...
            await SaveeOperation(bookPublishers, bookPublisherRepository);
            logger.LogTrace("Сохранение изданий книг");
        }

        private async Task<List<CoverType>> CoverTypeInit()
        {
            ...
            await SaveeOperation(coverTypes, coverTypeRepository);
            logger.LogTrace("Сохранение тип обложки");
            return coverTypes;
        }
        private async Task<List<Publisher>> PublisherInit()
        {
            ...
            await SaveeOperation(publishers, publisherRepository);
            logger.LogTrace("Сохранение изданий");
            return publishers;
        }


        private async Task SaveeOperation<TEntity>(List<TEntity> entities, IRepository<TEntity> repository)
            where TEntity : class, IEntity
        {
            for (int i = 0; i < entities.Count; i++)
            {
                await repository.CreateAsync(entities[i]);
            }

            await repository.SaveAsync();
        }

        private async Task SaveeOperation<TEntity, TId>(List<TEntity> entities, IRepository<TEntity, TId> repository)
